...
% p=0
%  VH_pol0=1/(1-beta*p)*(2*log((Ytil-xbar_c)/2)+beta*(1-p)*(1-beta*p+2*(1-p)*beta)/(1-2*beta*p-beta^2*(1-2*p))*(log((Ytil-xbar_c)/2))...
%         +beta*(1-p)*(1-beta*p)/(1-2*beta*p-beta^2*(1-2*p))*log(xbar_c));
%   
%     p=1
%  VH_pol1=1/(1-beta*p)*(2*log((Ytil-xbar_c)/2)+beta*(1-p)*(1-beta*p+2*(1-p)*beta)/(1-2*beta*p-beta^2*(1-2*p))*(log((Ytil-xbar_c)/2))...
%         +beta*(1-p)*(1-beta*p)/(1-2*beta*p-beta^2*(1-2*p))*log(xbar_c));

%figure(1)
%plot(lambda_vec,(ctilL_PL),'r', lambda_vec,(ctilH_PL),'b',lambda_vec, gtil_PL,'k')
n1=1;
n2=nlam;
n2p=11;
n2p=nlam
